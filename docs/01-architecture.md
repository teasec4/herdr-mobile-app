...
- **Плагин herdr** — тонкая обёртка: регистрирует событие
  ...
  «показать QR/ссылку для пары», запуск релея.
...
- **A + B1 = MVP.** Покрывают личный сценарий «ушёл из дома»: телефон с
  ...
  инфраструктуры, низкая задержка.
...
сменилась). Поллинг снимка раз в 1–2 c остаётся как fallback, если событие не
дошло.

## Поток данных «живого терминала»

- Чтение: приложение периодически (или по событию `agents.changed`) запрашивает
  `agent.output` — срез последних N строк; рендер и автоскролл на клиенте.
  Это «лайв-похоже», но не суб-100мс PTY, как у moshi через SSH.
