...
Когда агент в статусе `blocked`, анализируется вывод для поиска вариантов ответа. Парсер использует несколько стратегий:
...
- `option1/option2` - просто через слэш
...
### Стратегия 5: Ключевые слова действий

Если в последних строках есть несколько (2-4) ключевых слов:

```
proceed, continue, skip, retry, cancel,
approve, reject, accept, decline
```

То создаются кнопки с этими действиями.
...
### Парсинг (agent_page.dart:435-565)
...
4. Применяет стратегии парсинга по приоритету
...
void _parseSuggestedActions(String output) {
  ...
  // 5. Попробовать найти ключевые слова действий
}
```
...
✅ **Точность** - парсит реальные варианты из вывода, а не придумывает  
✅ **Приоритеты** - inline опции важнее ключевых слов  
